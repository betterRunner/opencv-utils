...
int main ()
{
    ...
    rng.fill(KF.statePost,RNG::UNIFORM,0,1);
    ...
    while (1)
    {
        ...
        Point predict_pt = Point(prediction.at<float>(0)*winWidth,prediction.at<float>(1)*winHeight);   //预测值(x',y')

        //3.update measurement
        measurement.at<float>(0) = (float)mousePosition.x / winWidth;
        measurement.at<float>(1) = (float)mousePosition.y / winHeight;
